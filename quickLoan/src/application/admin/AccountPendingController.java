...
public class AccountPendingController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
	 ...
	 public void loadPendingAccounts() {
	        List<Account> pendingAccounts = getPendingAccountsFromDatabase(); 
	        for (Account account : pendingAccounts) {
	            try {
	                FXMLLoader loader = new FXMLLoader(getClass().getResource("rowAccountPending.fxml"));
	                Parent row = loader.load();
	                RowAccountPending rowController = loader.getController();
	...
	public List<Account> getPendingAccountsFromDatabase() {
	    List<Account> pendingAccounts = new ArrayList<>();
	    String sql = "SELECT * FROM account WHERE status = false ORDER BY id DESC"; 
